            ...
            switch (vars[0]) {
                ...
                case "SNbL": //SNbL,number columns,key,fileName.extension
                    for (int i = 0; i < Integer.parseInt(vars[1]); i++) {
                        ...
                        int sizeOfSpriteX = (loaded.getWidth() / Integer.parseInt(vars[1]));
                        temp.getGraphics().drawImage(loaded.getSubimage(i * sizeOfSpriteX, 0, sizeOfSpriteX, loaded.getHeight()), 0, 0, null);
                ...
                case "GNbL":  //GNbL,number columns,number rows,key,fileName.extension
                    for (int y = 0; y < Integer.parseInt(vars[2]); y++) {
                        for (int x = 0; x < Integer.parseInt(vars[1]); x++) {
                            ...
                            int sizeOfSpriteX = (loaded.getWidth() / Integer.parseInt(vars[1]));
                            int sizeOfSpriteY = (loaded.getHeight() / Integer.parseInt(vars[2]));
                            temp.getGraphics().drawImage(loaded.getSubimage(x * sizeOfSpriteX, y * sizeOfSpriteY, sizeOfSpriteX, sizeOfSpriteY), 0, 0, null);
